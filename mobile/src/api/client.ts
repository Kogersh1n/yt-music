 ...
 * Жив ли бэкенд.
 *
 * Бросает исключение, а не возвращает false. Раньше отказ проглатывался,
 * и для React Query это выглядело успешным ответом «сервера нет» — то есть
 * повторять было нечего, а результат кэшировался на минуту. Один медленный
 * первый запрос на холодной сети, и приложение на минуту уходило
 * в демо-режим с пустой медиатекой.
 *
 * Срок ожидания тоже был мал: четыре секунды не покрывают холодное
 * подключение с рукопожатием TLS на мобильной сети — именно там, где
 * проверка и случается, сразу после запуска.
 */
export async function ping(): Promise<boolean> {
  await request<unknown>('/', { timeoutMs: 10_000 });
  return true;
}
