 ...
 * Поиск по YouTube.
 *
 * Поиск по своей медиатеке идёт локально, по уже загруженному кэшу, — так он
 * мгновенный и работает без сети. Серверный `/songs/search` починен и отвечает,
 * но нужен станет только когда медиатека перестанет помещаться в память.
 */
