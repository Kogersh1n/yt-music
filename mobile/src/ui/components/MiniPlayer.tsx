...
interface MiniPlayerProps {
  /**
   * Полоска стоит сама по себе, а не над панелью вкладок.
   *
   * Внутри вкладок отступ под системную полосу навигации даёт таб-бар;
   * на экранах очереди и текста его нет, и без этого мини-плеер упирается
   * в жест-полосу внизу экрана.
   */
  standalone?: boolean;
}

export const MiniPlayer = memo(function MiniPlayer({ standalone = false }: MiniPlayerProps) {
  ...
  return (
    <View style={[styles.wrapper, standalone && { paddingBottom: insets.bottom }]}>
      <ProgressLine />
